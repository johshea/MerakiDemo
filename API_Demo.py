...
#start Menu - Simple Structure for SEs without programming experiance
def print_menu():
...
while loop:
    ...
    if choice==1:
        ...
        pprint.pprint(netinput)
         ...
         #focus on serial # as a variable build options handler later build routine to extend network assignment
         newser = input("Claim your new Serial #")
         newser = str(newser)
         serinput = MerakiAPI.claim(apikey, netid, newser, licensekey, licensemode, orderid)
         pprint.pprint(serinput)
         print("Serial {0} has been added to Orginization {1}".format(str(newser), str(orgid)))
